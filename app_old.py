...
def build_summary_table(rows: list[dict], show_week: bool = False) -> html.Table:
    ...
    for row in rows:
        ...
        if data is None:
            ...
            day_arrow  = 'u' if chg_pct > 0 else ('d' if chg_pct < 0 else '-')
            ...
            # CONVICTION cell — badge + grey age stamp
            conv_cell = html.Td([
            ...
            # ACTION cell — instruction text + grey age stamp
            action_cell = html.Td([
            ...
            if rs_trend is not None:
                rs_arrow = 'up' if rs_trend > 1.5 else ('dn' if rs_trend < -1.5 else '->')
                rs_color = GREEN if rs_trend > 1.5 else (RED if rs_trend < -1.5 else YELLOW)
                rs_sign  = '+' if rs_trend >= 0 else ''
                rs_cell = html.Td([
                    html.Div(f'{rs_arrow} {rs_sign}{rs_trend:.1f}%', style={'color': rs_color, 'fontWeight': '700', 'fontSize': '13px'}),
